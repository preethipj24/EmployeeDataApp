...
const app = express();
const port = process.env.PORT || 3000; // Use process.env.PORT for dynamic port on hosting
...
app.post('/api/employees', async (req, res) => {
    ...
    try {
        const query = `
            ...
            ) RETURNING id; -- RETURNING id is specific to PostgreSQL for getting the new ID
        `;
        const values = [
            employeeData.first_name, employeeData.middle_name, employeeData.last_name,
            employeeData.department, employeeData.designation, employeeData.location,
            ...
            employeeData.date_of_birth, employeeData.marital_status,
            employeeData.ec_person_name, employeeData.ec_contact_number, employeeData.ec_relationship,
            JSON.stringify(employeeData.dependents_details || []), // pg handles JSON strings for JSONB
            employeeData.highest_education,
            JSON.stringify(employeeData.education_details || []),
            JSON.stringify(employeeData.work_experience_details || []),
            employeeData.signature_image_data,
            declarationDate,
            declarationPlace
        ];

        const result = await pool.query(query, values);
        res.status(201).json({ message: 'Employee data saved successfully!', id: result.rows[0].id });
    } catch (error) {
        console.error('Error saving employee data:', error);
        res.status(500).json({ message: 'Error saving employee data.', error: error.message });
    }
...
app.get('/api/employees/:id', async (req, res) => {
    ...
    try {
        ...
        const employee = {
            ...result.rows[0],
            dependents_details: safeParseJSON(result.rows[0].dependents_details),
            education_details: safeParseJSON(result.rows[0].education_details),
            work_experience_details: safeParseJSON(result.rows[0].work_experience_details)
        };
        res.status(200).json(employee);
    } catch (error) {
        console.error('Error fetching single employee data:', error);
...
app.get('/api/employees/:id/pdf', async (req, res) => {
    ...
    try {
        ...
        // html-pdf options for better rendering fidelity
        const pdfOptions = {
            format: 'A4',
            orientation: 'portrait',
            border: '1cm',
            // You can add more options here if needed, e.g., for header/footer
            // header: {
            //     height: '20mm',
            //     contents: '<div style="text-align: center;">Employee Report</div>'
            // },
            // footer: {
            //     height: '20mm',
            //     contents: {
            //         first: 'Cover page',
            //         2: 'Second page', // Any page number can be used.
            //         default: '<span style="color: #444;">{{page}}</span>/<span>{{pages}}</span>', // fallback for all other pages
            //         last: 'Last Page'
            //     }
            // }
        };
        ...
        pdf.create(pdfHtml, pdfOptions).toBuffer((err, buffer) => {
            if (err) {
                console.error('Error generating PDF:', err);
                return res.status(500).json({ message: 'Error generating PDF.', error: err.message });
            }
