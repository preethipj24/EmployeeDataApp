...
// Serve static files (your HTML, CSS, JS) from the current directory
// IMPORTANT: On Render, this might not be needed if Netlify hosts the frontend.
// This is primarily for local development or if Render serves both.
app.use(express.static(__dirname));
...
const pool = mysql.createPool({
    ...
    database: process.env.DB_NAME, // Changed from 'database' to 'DB_NAME' for clarity
    waitForConnections: true,
    connectionLimit: 10,
    queueLimit: 0,
    ssl: { // Add SSL configuration for PlanetScale
